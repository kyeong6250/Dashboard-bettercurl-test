...
authenticated headers, and a JSON POST body, all against
[GitHub's REST API](https://docs.github.com/en/rest).

Every read-only request file here was run for real (not just written to
look plausible) while building this repo.
...
Run all four read-only ones in sequence:
...
`requests/create-issue.example.yml` is deliberately a *template*, not
something meant to run as-is — running it for real creates an actual issue
on a real repo, and it needs a real token to do that. To try it:

1. Copy it: `cp requests/create-issue.example.yml requests/create-issue.yml`
   (`create-issue.yml` is gitignored, so a real token in it can never get
   committed by accident)
2. Generate a [fine-grained personal access
   token](https://github.com/settings/tokens) with `Issues: write` on the
   target repo
3. Edit `requests/create-issue.yml`: replace `YOUR_GITHUB_TOKEN_HERE` with
   that token, and point `url:` at a repo you actually own or can write to
4. `bettercurl -f requests/create-issue.yml`

This file is also the one that demonstrates why bettercurl's spec uses
`json:` rather than `data:` for request bodies — see [Better-curl's
README](https://github.com/kyeong6250/Better-curl#json-vs-data--read-this-before-sending-a-postput-body)
